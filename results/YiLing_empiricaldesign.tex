...
The casualty strategy is designed to compare the returns of the worker and the two types of entrepreneurs.

First, we can calculate the returns following the same logit from \citet{moskowitz2002returns}
\begin{equation}
R_{t,t+1} = \frac{\text{Equity}_{t+1} + 1 \times \text{Capital Gain}_{t+1}}{\text{Equity}_{t+1}} \label{eq:returns}
\end{equation}

Then for each household, we have the returns each year either from business or from salaried work.

Second, since both CPS and PSID provide annual information about their class of work, we can identify the year of the household starting the business. Therefore, an event study can be designed to see the impact of starting a business on the returns to work , and set up 6-year windows, comparing the returns 3 years before they chose to be an entrepreneur, and 3 years after they chose to become an entrepreneur.

Specifically, the strategy is set in this way (corrected formula based on previous conversation):
\begin{equation}
